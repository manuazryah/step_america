<form method="get" class="form-container" id="user-chat">
    <input type="hidden" name="Chat[user_id]" value="<?= $user->id ?>" />
        ...
        <?php foreach ($model as $message) {
            ...
            <div class="direct-chat-msg <?= $message->message_status == 2 ? 'right' : '' ?>">
                <div class="direct-chat-info clearfix">
                    <span class="direct-chat-name <?= $message->message_status == 2 ? 'pull-right' : 'pull-left' ?>"><?= $message->message_status == 2 ? 'Admin' : Yii::$app->user->identity->user_name ?></span>
